...
public interface VoucherRepository extends JpaRepository<Voucher, Long> {
  ...
  // Voucher còn hiệu lực theo thời gian cho FE/user: chỉ lấy trangThai = 1
  @Query("""
  ...
  // Tìm theo trạng thái thời gian cho API "co-san"
  @Query("""
      SELECT v FROM Voucher v
      WHERE v.trangThai = 1
        AND (:now BETWEEN v.batDauLuc AND v.ketThucLuc)
  ...
  // Lấy tất cả voucher còn tồn tại (không bao gồm voucher đã hủy), sắp xếp theo
  // ngày tạo mới nhất
