...
public class VoucherService {
    ...
    public VoucherValidationResult validateVoucher(String maVoucher, Long khachHangId, BigDecimal donHangTong) {
        ...
        // Nếu voucher có trangThai = 1, cho phép sử dụng ngay cả khi hết hạn
        if (voucher.getKetThucLuc() != null && now.isAfter(voucher.getKetThucLuc())) {
            // Nếu trangThai = 0, không cho phép sử dụng
        ...
        if (voucher.getDonToiThieu() != null && donHangTong.compareTo(voucher.getDonToiThieu()) < 0) {
            return VoucherValidationResult
                    .invalid(String.format("Đơn hàng phải tối thiểu %s VNĐ", voucher.getDonToiThieu()));
        }
    ...
    public VoucherApplicationResult applyVoucher(String maVoucher, Long khachHangId, BigDecimal donHangTong,
        ...
        // Lưu lịch sử sử dụng (chỉ khi có khachHangId) - không critical, nên bọc
        // try-catch
        if (khachHangId != null) {
            try {
    ...
    @Transactional(propagation = org.springframework.transaction.annotation.Propagation.REQUIRES_NEW, noRollbackFor = {
    ...
    public Voucher updateVoucher(Long id, VoucherUpdateRequest request) {
        ...
        voucher.setKetThucLuc(request.getKetThucLuc());
        voucher.setGioiHanSuDung(request.getGioiHanSuDung());
        // Khi admin cập nhật từ màn hình quản lý, mặc định đưa voucher về trạng thái
        // Active
    ...
    public void deleteVoucher(Long id) {
        ...
        // Soft delete: đánh dấu voucher đã hủy, không xóa vật lý để tránh lỗi FK với
        // đơn hàng
    ...
    public String getTrangThaiHienThi(Voucher voucher) {
        ...
        if (voucher.getTrangThai() != null && voucher.getTrangThai() == 1) {
            ...
            if (voucher.getKetThucLuc() != null
                    && (now.isBefore(voucher.getKetThucLuc()) || now.isEqual(voucher.getKetThucLuc()))) {
                return "Đang diễn ra";
            }
        }
