      ...
      <div class="modal-content" @click.stop>
            ...
            <div class="row g-3">
              ...
              <div class="col-md-6">
                <label class="form-label">
                  Danh mục cha
                  <small v-if="currentParent" class="text-muted ms-1"
                    >— Đang thuộc: {{ currentParent.name }}</small
                  >
                </label>
        ...
        <div class="modal-footer">
          ...
          <button
            ...
            @click="
              editingCategory && !editingCategory.isLocal ? updateCategory() : createCategory()
            "
          >
...
const filteredRootCategories = computed(() => {
  ...
  if (selectedLevel.value)
    filtered = filtered.filter((c) => c.level === parseInt(selectedLevel.value))
  return filtered
})
...
const getStatusClass = (s) =>
  ({ active: 'bg-success', inactive: 'bg-warning' })[s] || 'bg-secondary'
...
const deleteCategory = async (cat) => {
  ...
  try {
    ...
    if (status === 409) {
      ...
      const related =
        typeof srv === 'number' && !Number.isNaN(srv) ? srv : Number(cat.productCount ?? 0)
...
const bulkUpdateStatus = (status) => {
  showConfirm(
    `Bạn có chắc chắn muốn cập nhật trạng thái cho ${selectedCategories.value.length} danh mục?`,
  ).then((ok) => {
    if (!ok) return
...
const bulkDelete = () => {
  showConfirm(`Bạn có chắc chắn muốn xóa ${selectedCategories.value.length} danh mục?`).then(
    (ok) => {
      if (!ok) return
...
const editCategory = async (cat) => {
  ...
  if (cat?.isLocal) {
    ...
    categoryForm.value = {
      ...
      status:
        cat.status === 'active' || cat.status === true || cat.status === 1 ? 'active' : 'inactive',
    }
    showAddModal.value = true
    return
  }
...
.root-node .node-content {
  ...
  border-left: 4px solid rgba(99, 102, 241, 0.15);
}
.root-node .node-name {
  font-size: 1.05rem;
  font-weight: 700;
...
.tree-node > .node-children .tree-node {
  border: none;
  background: transparent;
  box-shadow: none;
  margin: 0.25rem 0 0 0.5rem;
}
.tree-node > .node-children .tree-node > .node-content {
